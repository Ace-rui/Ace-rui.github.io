...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time)
{
    ...
    //EQUALIZ=1表示太亮或太暗，则cv::createCLAHE()自适应直方图均衡化处理
    if (EQUALIZE)
    {
        //自适应直方图均衡
    ...
    //此时forw_pts还保存的是上一帧图像中的特征点，所以清除
    forw_pts.clear();
    ...
    if (cur_pts.size() > 0)
    {
        ...
        vector<uchar> status;
        vector<float> err;
        //对前一帧特征点cur_pts进行LK金字塔光流跟踪，得到forw_img
        ...
        for (int i = 0; i < int(forw_pts.size()); i++)
            if (status[i] && !inBorder(forw_pts[i]))
                status[i] = 0;
        //根据status，剔除跟踪失败和图像边界处的点
        //不仅要从当前帧数据forw_pts中剔除，而且还有从cur_un_pts，prev_pts和cur_pts中剔除
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        if (n_max_cnt > 0)
        {
            ...
            //寻找新的特征点补齐
            //forw_img：输入图像，单通道。n_pts：保存检测出的角点。角点数目最大值。0.01：角点的品质因子。
            //MIN_DIST：对于初选出的角点而言，如果在其周围minDistance范围内存在其他更强角点，则将此角点删除
            //mask：制定感兴趣区域。计算shi-tomasi角点
...
//通过F矩阵去除outliners
//首先将将图像坐标畸变矫正后转换为像素坐标，通过cv::findFundamentalMat()计算F矩阵，利用得到的status通过reduceVector()去除outliers 
