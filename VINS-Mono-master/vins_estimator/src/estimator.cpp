...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const std_msgs::Header &header)
{
    ...
    //参数：frame_count窗口内帧的个数，image某帧所有特征点的[camera_id,[x,y,z,u,v,vx,vy]]构成的map,索引为feature_id；td相机和IMU同步校准得到的时间差
...
bool Estimator::initialStructure()
{
    ...
    //通过加速度标准差判断IMU是否有充分运动，检测IMU可观性
    {
        ...
        //因为没有算上all_image_frame的第一帧，所以求均值和标准差的时候要减一
        Vector3d aver_g;
    ...
    //将f_manager.feature保存到存有SFMFeature对象的sfm_f中
    ...
    //relativePose()由E矩阵恢复当前帧到第l帧的R，t
    Matrix3d relative_R;
    Vector3d relative_T;
    int l;
    //判断每帧到窗口最后一帧对应特征点的平均视差大于30，且内点数目大于12则可进行初始化，同时返回当前帧到第l帧的坐标系变换R和T
    ...
    if(!sfm.construct(frame_count + 1, Q, T, l,
              ...
              sfm_f, sfm_tracked_points))  //纯视觉sfm，求解窗口中所有图像帧的位姿QT（相对于第l帧）和特征点坐标sfm_tracked_points
    {
        //求解失败则边缘化最早一帧并滑动窗口
...
//实现了陀螺仪的偏置校准(加速度偏置没有处理)，计算速度V[0:n]、重力g、尺度s
bool Estimator::visualInitialAlign()
{
    ...
    //通过将重力旋转到z轴上，得到世界坐标系与相机坐标系c0之间的旋转矩阵rot_diff
